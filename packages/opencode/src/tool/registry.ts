...
import {
  TeamCreateTool, TeamSpawnTool, TeamMessageTool, TeamBroadcastTool,
  TeamTasksTool, TeamClaimTool, TeamApprovePlanTool, TeamShutdownTool,
  TeamCleanupTool, TeamStatusTool, TeamModeSetTool, TeamPermissionResponseTool,
} from "./team"
...
export namespace ToolRegistry {
  ...
  > = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // Agent teams tools
      const teamCreate = yield* build(TeamCreateTool)
      const teamSpawn = yield* build(TeamSpawnTool)
      const teamMessage = yield* build(TeamMessageTool)
      const teamBroadcast = yield* build(TeamBroadcastTool)
      const teamTasks = yield* build(TeamTasksTool)
      const teamClaim = yield* build(TeamClaimTool)
      const teamApprovePlan = yield* build(TeamApprovePlanTool)
      const teamShutdown = yield* build(TeamShutdownTool)
      const teamCleanup = yield* build(TeamCleanupTool)
      const teamStatus = yield* build(TeamStatusTool)
      const teamWait = yield* build(TeamWaitTool)
      const teamModeSet = yield* build(TeamModeSetTool)
      const teamPermissionResponse = yield* build(TeamPermissionResponseTool)
      const teamMemoryWrite = yield* build(TeamMemoryWriteTool)
      const teamMemoryRead = yield* build(TeamMemoryReadTool)
      ...
      const all = Effect.fn("ToolRegistry.all")(function* (custom: Tool.Info[]) {
        ...
        return [
          ...
          ...(Flag.OPENCODE_EXPERIMENTAL_AGENT_TEAMS
            ? [teamCreate, teamSpawn, teamMessage, teamBroadcast, teamTasks, teamClaim,
               teamApprovePlan, teamShutdown, teamCleanup, teamStatus, teamWait,
               teamModeSet, teamPermissionResponse]
            : []),
          ...(Flag.OPENCODE_TEAM_MEMORY ? [teamMemoryWrite, teamMemoryRead] : []),
          ...custom,
        ]
      })
