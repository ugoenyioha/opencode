  ...
  subagent_type: z
    ...
    .describe(
      "The type of specialized agent to use for this task. " +
        ...
        "Requires OPENCODE_FORK_SUBAGENT=1.",
    )
    .optional(),
  task_id: z
    .string()
...
export const TaskTool = Tool.define("task", async (ctx?) => {
  ...
  return {
    ...
    async execute(params: z.infer<typeof parameters>, ctx) {
      ...
      // Fork mode: no subagent_type → fork current session
      if (!params.subagent_type) {
        ...
        ctx.metadata({ title: params.description, metadata: { sessionId: forked.id, fork: true } })
        ...
        const result = await SessionPrompt.prompt({
          messageID, sessionID: forked.id, model: forkModel, agent: ctx.agent ?? "general", parts: promptParts,
        })
        const text = result.parts.findLast((x: MessageV2.Part) => x.type === "text")?.text ?? ""
        return {
          title: params.description,
          metadata: { sessionId: forked.id, fork: true },
          output: [`task_id: ${forked.id} (forked session)`, "", "<task_result>", text, "</task_result>"].join("\n"),
        }
      }
