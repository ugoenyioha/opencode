...
You can configure runtime limits under `server.limits`.
  ...
  "server": {
    "limits": {
      ...
      "rate_limit_backend": {
        "driver": "sqlite",
        "sqlite": {
          "path": "/var/lib/opencode/rate-limit.db"
        }
      }
    }
  }
...
- `rate_limit_backend.driver` - Backend driver for rate-limit state. Supported values: `sqlite` and `memory`. Default: `sqlite`.
- `rate_limit_backend.sqlite.path` - Optional sqlite file path override for rate-limit state.
...
You can configure sandboxing for bash execution backends through `sandbox`.
...
Supported `sandbox.bash` modes:

- `none` - Run bash without a sandbox.
- `namespace` - Linux namespaces, or `sandbox-exec` on macOS.
- `bwrap` - Linux bubblewrap backend.
- `gvisor` - Linux gVisor backend (requires `runsc`).
- `firecracker` - Linux Firecracker backend.
- `auto` - Pick the best available backend for the host.

Linux `auto` selection order:

- `firecracker -> gvisor -> bwrap -> namespace -> none`

Network is blocked by default for sandboxed bash runs. Set `sandbox.network` to `true` to opt in to network access.

When network is restricted (`sandbox.network: false`), this isolation extends to OpenCode's internal host process as well, blocking the `webfetch` tool and preventing the agent from exfiltrating data via HTTP hooks.

:::tip
Sandbox settings can be overridden on a per-agent basis to apply strict restrictions to specific subagents. See the [Agent Sandbox Configuration](/docs/agents#sandbox) for more details.
:::

:::note
On macOS, sandboxing uses `sandbox-exec`. Apple has deprecated it, but it is still used as the current backend. For Linux backend validation (`bwrap`, `gvisor`, `firecracker`), prefer running OpenCode inside OrbStack or another Linux environment.
:::

#### Validation matrix

Use these commands to validate sandbox and rate-limit behavior:

```bash
bun test test/sandbox/firecracker.test.ts test/sandbox/gvisor.test.ts test/sandbox/bwrap.test.ts
bun test test/tool/sandbox-security.test.ts test/tool/bash.test.ts
bun test test/server/rate-limit.test.ts test/server/rate-limit-store.test.ts
```
...
| ---------------- | ------- | ------------------------------------------------------------------- |
| `max_turns`      | number  | Maximum LLM call turns per session. Session stops when reached.     |
| `max_budget_usd` | number  | Maximum cost in USD per session. Warns at 80%, stops when exceeded. |
| `suggestions`    | boolean | Enable prompt suggestions after each response (default: enabled).   |
