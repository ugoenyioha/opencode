...
## Background task keybinds

| Shortcut  | Action                                          |
| --------- | ----------------------------------------------- |
| `ctrl+b`  | Send a running bash command to the background   |

:::note
In **tmux**, press `ctrl+b` twice since the first press is consumed by the tmux prefix key. When no bash command is running, `ctrl+b` acts as cursor-left (Readline/Emacs-style).
:::

[Learn more](/docs/background-tasks) about background tasks.

---

## Agent team keybinds

These keybinds are available when [agent teams](/docs/agent-teams) are enabled.
