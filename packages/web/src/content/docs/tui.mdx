...
Browse and edit instruction files and project memory.

```bash frame="none"
/memory
```

Opens a dialog showing all instruction files (AGENTS.md, CLAUDE.md), rules directories (`.opencode/rules/`, `.claude/rules/`), and global files. Select a file to open it in your editor. [Learn more](/docs/memory).
...
Opens the background tasks dialog. Shows all running, completed, and failed tasks. Select a task to view its output, press `Ctrl+K` to kill a running task. [Learn more](/docs/background-tasks).
