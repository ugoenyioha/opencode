...
OpenCode automatically caches your shell environment at startup so the LLM doesn't have to evaluate heavy dotfiles (`.zshrc`, `.bash_profile`, `nvm`, `pyenv`, etc.) on every single bash command. This dramatically reduces latency and speeds up agents that run lots of shell operations.
:::
...
Save persistent facts to project memory.
...
Persists facts and preferences to `.opencode/rules/memory.md`, which is loaded in all future sessions. Use this when you want the agent to remember something across sessions. [Learn more](/docs/memory).

---

### process_query

Query and interact with background tasks.
...
Lets the agent check status, read output, and search through background task output. Supports actions: `list`, `status`, `read_output`, `read_last_n`, and `search`. [Learn more](/docs/background-tasks).

---

### Team tools (experimental)

A set of tools for coordinating agent teams. These are only available when `OPENCODE_EXPERIMENTAL_AGENT_TEAMS=1` is set.

| Tool                | Description                                                |
| ------------------- | ---------------------------------------------------------- |
| `team_create`       | Create a new agent team                                    |
| `team_spawn`        | Spawn a teammate in a new session                          |
| `team_message`      | Send a message to a teammate                               |
| `team_broadcast`    | Broadcast a message to all teammates                       |
| `team_tasks`        | Manage the shared task list                                |
| `team_claim`        | Claim a pending task                                       |
| `team_status`       | Get a full snapshot of members, tasks, and unread messages |
| `team_approve_plan` | Approve/reject a teammate's plan                           |
| `team_shutdown`     | Request a teammate to shut down                            |
| `team_cleanup`      | Clean up team resources                                    |

[Learn more](/docs/agent-teams) about agent teams.
