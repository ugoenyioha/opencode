...
Use the `isolation` config to execute the agent in an isolated environment. Set this to `"worktree"` to spawn the agent in a dedicated git worktree branch.
...
```json title="opencode.json" {4}
{
  "agent": {
    "experiment": {
      "isolation": "worktree",
      "description": "Tests experimental approaches without modifying your working tree"
    }
  }
}
```

This is especially useful for [Agent Teams](/docs/agent-teams) when running multiple subagents in parallel to prevent file collisions or accidental overwrites of your uncommitted work.

---

### Sandbox

You can override global [Sandbox](/docs/config#sandbox) configurations on a per-agent basis. This allows you to apply strict restrictions to specific agents while keeping your primary agents unconstrained.

Agent-level overrides can only make the sandbox **more restrictive**, never less. For example, if the global `sandbox.network` is `false`, an agent cannot enable it.
...
```json title="opencode.json" {4-7}
{
  "agent": {
    "untrusted-experiment": {
      ...
      "description": "Runs code in an isolated and networked-blocked environment"
    }
  }
}
```
