...
<Card title="Tool endpoint" icon="server" href="/tool-endpoint">
  Expose selected tools over HTTP with allowlists and auth.
</Card>

<Card title="HTTP compat" icon="globe" href="/compat-http">
  Use OpenAI-compatible and Anthropic-compatible routes with direct `provider/model` IDs.
</Card>
